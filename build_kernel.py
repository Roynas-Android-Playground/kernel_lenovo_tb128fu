...
def main():
    ...
    parser.add_argument('--permissive', action='store_true', help="Build SELinux permissive kernel")
    parser.add_argument('--audit', action='store_true', help="Build SELinux audit enabled kernel")
    ...
    make_defconfig += ['grass-perf_defconfig']
    if not args.audit:
        make_defconfig += ['noaudit.config']
    if args.permissive:
    ...
    zipname = 'GrassKernel_tb128fu_{}.zip'.format(datetime.today().strftime('%Y-%m-%d'))
    os.chdir('AnyKernel3/')
    zip_files(zipname, [
        'Image', 
