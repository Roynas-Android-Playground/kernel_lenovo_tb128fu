...
//+bug686781, shenwenbin.wt, add, 20210818, modify bias power setting to interface config
#define ocp2138_VPOS_ADDRESS		0x00
#define ocp2138_VNEG_ADDRESS		0x01
//#define ocp2138_DIS_ADDRESS			0x03
//#define ocp2138_CONTROL_ADDRESS		0xFF
...
//+OAK8,shenwenbin.wt,MOD,20211120,improve ocp2138 driver
...
int ocp2138_set_voltage(LCM_LDO_VOLTAGE_E avdd, LCM_LDO_VOLTAGE_E avee,u32 pwrup_delay)
{
	...
	//u8 active_discharge = 0x03;
	//u8 ctrl_register_value = 0xf0;
	...
	*/
	...
	if(( reg_value_avdd == LCM_LDO_VOL_DEFAULT &&  reg_value_avee == LCM_LDO_VOL_DEFAULT) ||(i2c_client == NULL))
	{
	//-OAK8,shenwenbin.wt,MOD,20211120,improve ocp2138 driver
	...
	/*
	if(0 != i2c_smbus_write_i2c_block_data(i2c_client, ocp2138_DIS_ADDRESS,1, &active_discharge))
		printk("LCD bias IC discharger disable/enable write fail\n");

	if(0 != i2c_smbus_write_i2c_block_data(i2c_client, ocp2138_CONTROL_ADDRESS, 1,&ctrl_register_value))
		printk("LCD bias IC CONTRO write fail\n");
	*/

	//+bug692935, shenwenbin.wt, add, 20210923, improve panel timing
	if(pwrup_delay)
		mdelay(pwrup_delay);
	else
		mdelay(8);
...
{
	if ( gpio_is_valid(GPIO_LCD_BIAS_ENN) )
		gpio_direction_output(GPIO_LCD_BIAS_ENN, 0);	//bug686781, shenwenbin.wt, modify, 20210917, modify bias powerdown timing
	if(pwrdown_delay)
		mdelay(pwrdown_delay);
	else
		mdelay(8);
	if ( gpio_is_valid(GPIO_LCD_BIAS_ENP) )
		gpio_direction_output(GPIO_LCD_BIAS_ENP, 0);	//bug686781, shenwenbin.wt, modify, 20210917, modify bias powerdown timing
	if(pwrdown_delay)
		mdelay(pwrdown_delay);
	else
		mdelay(8);
...
{   
	...
	/* enn, enp gpio info */
	GPIO_LCD_BIAS_ENN = of_get_named_gpio(np, "ocp2138,lcd-bias-enn-gpio", 0);
	if (!gpio_is_valid(GPIO_LCD_BIAS_ENN))
	{
		LCM_LOGE("ocp2138_parse_dt() ennnnn gpio not specified\n");
	}

	GPIO_LCD_BIAS_ENP = of_get_named_gpio(np, "ocp2138,lcd-bias-enp-gpio", 0);
	if (!gpio_is_valid(GPIO_LCD_BIAS_ENP))
	{
		LCM_LOGE("ocp2138_parse_dt() enpppp gpio not specified\n");
	}
...
static int ocp2138_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("ocp2138_i2c_probe begin\n");
