...
struct proc_dir_entry *himax_proc_panel_direction_file;
struct proc_dir_entry *himax_proc_game_mode_file;	//OAK4146,shenwenbin.wt,ADD,20220125,add TP game mode
struct proc_dir_entry *himax_proc_prevent_notch_file;//OAK4139,shenwenbin.wt,ADD,20220217,improve virtual bar edge restrain
...
static int himax_panel_direction_show(struct seq_file *s, void *v)
{
	uint8_t direction = g_core_fp.fp_read_panel_direction();
	ssize_t ret = 0;

	seq_printf(s,"direction= %d\n", direction);
	switch(direction)
	{	
		case PANEL_PORTRAIT_0:
			seq_puts(s, "Panel direction is PORTRAIT\n");
			break;
		case PANEL_LANDSCAPE_90:
			seq_puts(s, "Panel direction is LANDSCAPE_90\n");
			break;
		case PANEL_LANDSCAPE_270:
			seq_puts(s, "Panel direction is LANDSCAPE_270\n");
			break;
		default:
			E("%s: cmd  is not match, pls check.\n", __func__);
			seq_puts(s, "Panel direction is UNKNOWN\n");
			break;
	}
	return ret;
}

static ssize_t himax_panel_direction_store(struct file *filp,
			const char __user *buff, size_t len, loff_t *data)
{
	char buf[3] = {0};
	
	if (len > 3) {
		E("%s: command  len exceeds 3 chars.\n", __func__);
		return -EFAULT;
	}
	
	if (copy_from_user(buf, buff, len)){
		E("%s: copy_from_user err.\n", __func__);
		return -EFAULT;
	}
	...
	{	
		case '1':
			g_core_fp.fp_set_panel_direction(PANEL_PORTRAIT_0);			
			break;
		case '2':
			g_core_fp.fp_set_panel_direction(PANEL_LANDSCAPE_90);	
			break;
		case '3':
			g_core_fp.fp_set_panel_direction(PANEL_LANDSCAPE_270);	
			break;
		default:
			E("%s: cmd  is not match, pls check.\n", __func__);
			break;
...
__CREATE_RW_NODE_HX(panel_direction);

//+OAK4146,shenwenbin.wt,ADD,20220125,add TP game mode
static int himax_game_mode_show(struct seq_file *s, void *v)
{
	ssize_t ret = 0;
	uint8_t game_mode = g_core_fp.fp_read_game_mode();

	seq_printf(s,"game mode = %d\n", game_mode);
	switch(game_mode)
	{	
		case 0:
			seq_puts(s, "game mode is off\n");
			break;
		case 1:
			seq_puts(s, "game mode is on\n");
			break;
		default:
			E("%s: cmd  is not match, pls check.\n", __func__);
			seq_puts(s, "game mode is UNKNOWN\n");
			break;
	}
	return ret;
}

static ssize_t himax_game_mode_store(struct file *filp,
			const char __user *buff, size_t len, loff_t *data)
{
	char buf[3] = {0};
	uint8_t cmd_tmp[3] = {0};	//OAK4146,shenwenbin.wt,ADD,20220224,add TP game mode
	...
	//+OAK4146,shenwenbin.wt,ADD,20220224,add TP game mode
	sscanf(buf,"%d",cmd_tmp);
	I("%s cmd is %d\n",__func__,cmd_tmp[0]);
	g_core_fp.fp_set_game_mode(cmd_tmp[0]);
	//-OAK4146,shenwenbin.wt,ADD,20220224,add TP game mode
	return len;
}
__CREATE_RW_NODE_HX(game_mode);
//-OAK4146,shenwenbin.wt,ADD,20220125,add TP game mode

//+OAK4139,shenwenbin.wt,ADD,20220217,improve virtual bar edge restrain
static int himax_prevent_notch_show(struct seq_file *s, void *v)
{
	ssize_t ret = 0;
	seq_printf(s, "cmd_parm = %d,%d,%d,%d\n", private_ts ->cmd_param[0], private_ts ->cmd_param[1], private_ts ->cmd_param[2], private_ts ->cmd_param[3]);
	return ret;
}

static ssize_t himax_prevent_notch_store(struct file *filp,
			const char __user *buff, size_t len, loff_t *data)
{
	char *tmp_buf;
	uint8_t buf[4] = {0};
	...
	if (!tmp_buf) {
		E("%s:Allocate tmp_buf fail!\n",__func__);
		ret = -ENOMEM;
		goto out;
	}

	if (copy_from_user(tmp_buf, buff, len)) {
		E("%s:copy_from_user error!\n",__func__);
		ret=  -EINVAL;
		goto out;
	}

	ret = sscanf(tmp_buf, "%d,%d,%d,%d", private_ts ->cmd_param, private_ts ->cmd_param+1, private_ts ->cmd_param+2, private_ts ->cmd_param+3);

	I("%s:cmd_parm = %d,%d,%d,%d\n", __func__, private_ts ->cmd_param[0], private_ts ->cmd_param[1], private_ts ->cmd_param[2], private_ts ->cmd_param[3]);
	
	buf[0] = (uint8_t)(private_ts ->cmd_param[3] & 0xFF);
	tmp1 = (uint8_t)(private_ts ->cmd_param[2] & 0x0F);
	tmp2 = (uint8_t)((private_ts ->cmd_param[3] >> 8) & 0xFF);
	buf[1] = (tmp1 << 4) | (tmp2 & 0x0F);
	buf[2] = (uint8_t)((private_ts ->cmd_param[2] >> 4) & 0xFF);
	
	tmp1 = (uint8_t)(private_ts ->cmd_param[1] & 0xFF);
	tmp2 = (uint8_t)(private_ts ->cmd_param[0] & 0xFF);
	if ( private_ts ->cmd_param[0] == 0 && private_ts ->cmd_param[1] == 0 && private_ts ->cmd_param[2] == 0 && private_ts ->cmd_param[3] == 0 ) {
		
		buf[3] = 0x00;
		
	} else {
		if ( tmp1 == 0x01 ) {
			buf[3] = 0xF0 | (tmp2 & 0x07);
		}else if ( tmp1 == 0x02 ) {
			buf[3] = 0xF8 | (tmp2 & 0x07);
		} else {
			buf[3] = 0x00;
		}
	}
		
	ret = g_core_fp.fp_set_prevent_notch(buf);
out:
	if (tmp_buf)
		kfree(tmp_buf);
	return len;
}
__CREATE_RW_NODE_HX(prevent_notch);
//-OAK4139,shenwenbin.wt,ADD,20220217,improve virtual bar edge restrain
			...
			const char __user *buff, size_t len, loff_t *data)
{
	...
	for (i = 0; i < 4; i++) {
		if (input_str[i] == NULL) {
			...
			//		"help") >= 0) {
				if (strstr(input_str[i], "help") != NULL) {
					is_flash = 0;
					goto CON_WORK;
			//	} else if (str_idx_of_str(input_str[i],
			//		"flash") >= 0) {
				} else if (strstr(input_str[i], "flash") != NULL) {
					is_flash = 1;
			//	} else if (str_idx_of_str(input_str[i],
			//		"ram") >= 0) {
				} else if (strstr(input_str[i], "ram") != NULL) {
					is_flash = 2;
				} else {
					is_flash = -1;
					goto CON_WORK;
			...
			//	if (str_idx_of_str(input_str[i],
			//		"print") >= 0) {
				if (strstr(input_str[i], "print") != NULL) {
					is_file = 2;
			//	} else if (str_idx_of_str(input_str[i],
			//		"file") >= 0) {
				} else if (strstr(input_str[i], "file") != NULL) {
					is_file = 1;
				} else {
					is_file = -1;
					goto CON_WORK;
...
int himax_touch_proc_init(void)
{
	...
	himax_proc_panel_direction_file = proc_create(HIMAX_PROC_PANEL_DIRECTION_FILE,
				0666, NULL,
				&himax_panel_direction_ops);
	//-OAK700,shenwenbin.wt,add,20211204,add charge mode
	if (himax_proc_panel_direction_file == NULL) {
		E(" %s: direction file create failed!\n", __func__);
		goto fail_5;
	}

	//+OAK4146,shenwenbin.wt,ADD,20220125,add TP game mode
	himax_proc_game_mode_file = proc_create(HIMAX_PROC_GAME_MODE_FILE,
				0666, NULL,
				&himax_game_mode_ops);
	if (himax_proc_game_mode_file == NULL) {
		E(" %s: game mode file create failed!\n", __func__);
		goto fail_6;
	}	
	//-OAK4146,shenwenbin.wt,ADD,20220125,add TP game mode

	//+OAK4139,shenwenbin.wt,ADD,20220217,improve virtual bar edge restrain
	himax_proc_prevent_notch_file = proc_create(HIMAX_PROC_PREVENT_NOTCH_FILE,
				0666, NULL,
				&himax_prevent_notch_ops);
	if (himax_proc_prevent_notch_file == NULL) {
		E(" %s: prevent notch file create failed!\n", __func__);
		goto fail_7;
	}
	//-OAK4139,shenwenbin.wt,ADD,20220217,improve virtual bar edge restrain
...
fail_7: remove_proc_entry(HIMAX_PROC_PREVENT_NOTCH_FILE, NULL);//OAK4139,shenwenbin.wt,ADD,20220217,improve virtual bar edge restrain
fail_6: remove_proc_entry(HIMAX_PROC_GAME_MODE_FILE, NULL);	//OAK4146,shenwenbin.wt,ADD,20220125,add TP game mode
fail_5: remove_proc_entry(HIMAX_PROC_PANEL_DIRECTION_FILE, NULL);	//OAK700,shenwenbin.wt,add,20211204,add charge mode
...
{
	remove_proc_entry(HIMAX_PROC_PREVENT_NOTCH_FILE, NULL);//OAK4139,shenwenbin.wt,ADD,20220217,improve virtual bar edge restrain
	remove_proc_entry(HIMAX_PROC_GAME_MODE_FILE, NULL);	//OAK4146,shenwenbin.wt,ADD,20220125,add TP game mode
	remove_proc_entry(HIMAX_PROC_PANEL_DIRECTION_FILE, NULL);	//OAK700,shenwenbin.wt,add,20211204,add charge mode
