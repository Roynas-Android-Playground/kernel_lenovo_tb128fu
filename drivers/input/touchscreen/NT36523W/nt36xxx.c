...
#include <linux/pm_runtime.h>	//OAK78,shenwenbin.wt,MOD,20211130,double wakeup sometime SPI not wake
#include <linux/hardware_info.h>	//Bug697290 ,shenwenbin.wt,ADD,20211021,add TP hardware info
...
//+OAK700,shenwenbin.wt,modify,20211227,modify panel direction black display issues
...
extern int32_t nvt_edge_grid_zone_set(void);
//-OAK4146,shenwenbin.wt,ADD,20220125,add TP game mode
...
#define USB_DETECT_OUT 2		//OAK700,shenwenbin.wt,modify,20220211,charger mode notify sometime error
...
void nvt_bootloader_reset(void)
{
	...
	ret = nvt_write_addr(SWRST_N8_ADDR, 0x69);
	if (ret) {
		NVT_ERR("SWRST_N8_ADDR write fail, ret = %d\n", ret);
	}
	...
	if (SPI_RD_FAST_ADDR) {
		/* disable SPI_RD_FAST */
		ret = nvt_write_addr(SPI_RD_FAST_ADDR, 0x00);
		if (ret) {
			NVT_ERR("SWRST_N8_ADDR write fail, ret = %d\n", ret);
		}
	}
...
int32_t nvt_get_fw_info(void)
{
	...
	int32_t ret = 0;
	char firmware_ver[HARDWARE_MAX_ITEM_LONGTH];	//Bug697290 ,shenwenbin.wt,ADD,20211021,add TP hardware info
...
info_retry:
	...
	//+Bug697290 ,shenwenbin.wt,ADD,20211021,add TP hardware info
	snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"NT36523-W,VND:TM,FW:0x%02x", ts->fw_ver);
	NVT_LOG("TP_name = %s\n", firmware_ver);
...
void nvt_set_charger_mode(void)
{
    ...
    if ((ts->charger_mode == USB_DETECT_IN)||(ts->charger_mode == 4)){
    ...
    } else if ((ts->charger_mode == USB_DETECT_OUT)||(ts->charger_mode == 3)) {
        buf[0] = EVENT_MAP_HOST_CMD;
...
static void nvt_set_charger_wqu(struct work_struct *work)
{
    //+OAK700,shenwenbin.wt,modify,20211227,modify panel direction black display issues
    mutex_lock(&ts->lock);
...
unsigned long val, void *v)
{
    ...
    if (!psy) {
        NVT_ERR("Couldn't get battery psy\n");
        return -EINVAL;
    }

    if (!strcmp(psy->desc->name, "battery")) {
        NVT_LOG("NVT_charger_notifier_callback event psy->desc->name is battery\n");
        if (psy && val == POWER_SUPPLY_PROP_STATUS) {
            ...
            if (ret < 0) {
                ...
                if (ts->charger_mode != prop.intval) {
                    ...
                    if ( bTouchIsAwake && (ts->nvt_charger_wq != NULL) && (ts->update_floating == 0))
                        queue_work(ts->nvt_charger_wq,&ts->nvt_charger_work);
                }
            }
        }
    }
...
*/
	...
	if (nvt_wdt_fw_recovery(point_data)) {
		...
		nvt_update_firmware(BOOT_UPDATE_FIRMWARE_NAME);
		wdt_recovery = true;	//OAK700,shenwenbin.wt,modify,20211227,modify panel direction black display issues
		goto XFER_ERROR;
	}
#endif /* #if NVT_TOUCH_WDT_RECOVERY */
...
//+OAK1911,shenwenbin.wt,ADD,20211228,add penraw node for customer
		...
		if((pen_status == TS_TOUCH) || (pen_status == TS_RELEASE)){
			...
			ppen_info->coords.x = pen_x/2;	//OAK1911,shenwenbin.wt,MOD,20220217,penraw have a line
			ppen_info->coords.y = pen_y/2;	//OAK1911,shenwenbin.wt,MOD,20220217,penraw have a line
...
static int8_t nvt_ts_check_chip_ver_trim(uint32_t chip_ver_trim_addr)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		ret = nvt_set_page(chip_ver_trim_addr);
		if (ret) {
...
//+OAK700,shenwenbin.wt,add,20211204,add charge mode	
		...
		ts->nvt_charger_wq = create_singlethread_workqueue("nvt_charger_wq");
		...
		INIT_WORK(&ts->nvt_charger_work, nvt_set_charger_wqu);
...
//-OAK1911,shenwenbin.wt,ADD,20211228,add penraw node for customer
	...
	INIT_WORK(&ts_restore_cmd_work, nvt_restore_cmd_func);	//OAK700,shenwenbin.wt,modify,20211227,modify panel direction black display issues
...
//-OAK1911,shenwenbin.wt,ADD,20211228,add penraw node for customer
	...
	cancel_work_sync(&ts_restore_cmd_work);	//OAK700,shenwenbin.wt,modify,20211227,modify panel direction black display issues
...
static int32_t __init nvt_driver_init(void)
{
	...
	NVT_LOG("start %s\n",__func__);
	if(!strstr(saved_command_line,"qcom,mdss_dsi_nt36523w_tm_2k_video") && !strstr(saved_command_line, "qcom,mdss_dsi_nt36523w_tm_2k_apcf_video")) {
