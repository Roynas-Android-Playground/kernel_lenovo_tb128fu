 ...
 *
 * $Revision: 85416 $
 * $Date: 2021-07-20 15:08:07 +0800 (週二, 20 七月 2021) $
 *
...
//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
#define SHORT_TEST_CSV_FILE "/data/vendor/fac_sources/ShortTest"
#define OPEN_TEST_CSV_FILE "/data/vendor/fac_sources/OpenTest"
#define FW_RAWDATA_CSV_FILE "/data/vendor/fac_sources/FWRawdataTest"
#define FW_CC_CSV_FILE "/data/vendor/fac_sources/FWCCTest"
#define NOISE_TEST_CSV_FILE "/data/vendor/fac_sources/NoiseTest"
#define PEN_FW_RAW_TEST_CSV_FILE "/data/vendor/fac_sources/PenFWRawTest"
#define PEN_NOISE_TEST_CSV_FILE "/data/vendor/fac_sources/PenNoiseTest"
//-bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
...
static int32_t TestResult_Final = 0;	//OAK497,shenwenbin.wt,MOD,20220112,selftest fail alway fail
...
static char test_csv_file_path[256];
//-bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
...
static int32_t nvt_read_baseline(int32_t *xdata)
{
	...
	//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
	snprintf(test_csv_file_path, sizeof(test_csv_file_path),
		 "%s-%04d%02d%02d_%02d%02d%02d.csv", FW_RAWDATA_CSV_FILE, 
		   rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		   rtc_tm.tm_hour, rtc_tm.tm_min,rtc_tm.tm_sec);
	if (nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, test_csv_file_path, 0) < 0) {
...
static int32_t nvt_read_CC(int32_t *xdata)
{
	...
	//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
	snprintf(test_csv_file_path, sizeof(test_csv_file_path),
		 "%s-%04d%02d%02d_%02d%02d%02d.csv", FW_CC_CSV_FILE, 
		   rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		   rtc_tm.tm_hour, rtc_tm.tm_min,rtc_tm.tm_sec);
	if (nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, test_csv_file_path, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
...
static int32_t nvt_read_pen_baseline(void)
{
	...
	//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
	snprintf(test_csv_file_path, sizeof(test_csv_file_path),
		 "%s-%04d%02d%02d_%02d%02d%02d.csv", PEN_FW_RAW_TEST_CSV_FILE, 
		   rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		   rtc_tm.tm_hour, rtc_tm.tm_min,rtc_tm.tm_sec);
	printk("%s:RawData_PenTipX_Raw\n", __func__);
	if (nvt_save_rawdata_to_csv(RawData_PenTipX_Raw, ts->x_num, ts->y_gang_num, test_csv_file_path, csv_output_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	csv_output_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
	printk("%s:RawData_PenTipY_Raw\n", __func__);
	if (nvt_save_rawdata_to_csv(RawData_PenTipY_Raw, ts->x_gang_num, ts->y_num, test_csv_file_path, csv_output_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	csv_output_offset += ts->y_num * ts->x_gang_num * 7 + ts->y_num * 2;
	printk("%s:RawData_PenRingX_Raw\n", __func__);
	if (nvt_save_rawdata_to_csv(RawData_PenRingX_Raw, ts->x_num, ts->y_gang_num, test_csv_file_path, csv_output_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	csv_output_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
	printk("%s:RawData_PenRingY_Raw\n", __func__);
	if (nvt_save_rawdata_to_csv(RawData_PenRingY_Raw, ts->x_gang_num, ts->y_num, test_csv_file_path, csv_output_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	//-bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
	snprintf(test_csv_file_path, sizeof(test_csv_file_path),
		 "%s-%04d%02d%02d_%02d%02d%02d.csv", NOISE_TEST_CSV_FILE, 
		   rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		   rtc_tm.tm_hour, rtc_tm.tm_min,rtc_tm.tm_sec);
	if (nvt_save_rawdata_to_csv(RawData_Diff_Max, X_Channel, Y_Channel, test_csv_file_path, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	//-bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
	...
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Diff_Min, X_Channel, Y_Channel, test_csv_file_path, rawdata_diff_min_offset) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	...
	if (ts->pen_support) {
		...
		//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
		snprintf(test_csv_file_path, sizeof(test_csv_file_path),
			 "%s-%04d%02d%02d_%02d%02d%02d.csv", PEN_NOISE_TEST_CSV_FILE, 
			   rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
			   rtc_tm.tm_hour, rtc_tm.tm_min,rtc_tm.tm_sec);
		
		printk("%s:RawData_PenTipX_DiffMax:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenTipX_DiffMax, ts->x_num, ts->y_gang_num, test_csv_file_path, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		printk("%s:RawData_PenTipX_DiffMin:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenTipX_DiffMin, ts->x_num, ts->y_gang_num, test_csv_file_path, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		printk("%s:RawData_PenTipY_DiffMax:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenTipY_DiffMax, ts->x_gang_num, ts->y_num, test_csv_file_path, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_num * ts->x_gang_num * 7 + ts->y_num * 2;
		printk("%s:RawData_PenTipY_DiffMin:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenTipY_DiffMin, ts->x_gang_num, ts->y_num, test_csv_file_path, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_num * ts->x_gang_num * 7 + ts->y_num * 2;
		printk("%s:RawData_PenRingX_DiffMax:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenRingX_DiffMax, ts->x_num, ts->y_gang_num, test_csv_file_path, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		printk("%s:RawData_PenRingX_DiffMin:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenRingX_DiffMin, ts->x_num, ts->y_gang_num, test_csv_file_path, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_gang_num * ts->x_num * 7 + ts->y_gang_num * 2;
		printk("%s:RawData_PenRingY_DiffMax:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenRingY_DiffMax, ts->x_gang_num, ts->y_num, test_csv_file_path, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		csv_pen_noise_offset += ts->y_num * ts->x_gang_num * 7 + ts->y_num * 2;
		printk("%s:RawData_PenRingY_DiffMin:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_PenRingY_DiffMin, ts->x_gang_num, ts->y_num, test_csv_file_path, csv_pen_noise_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
		//-bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
...
static int32_t nvt_read_fw_open(int32_t *xdata)
{
	...
	//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
	snprintf(test_csv_file_path, sizeof(test_csv_file_path),
		 "%s-%04d%02d%02d_%02d%02d%02d.csv", OPEN_TEST_CSV_FILE, 
		   rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		   rtc_tm.tm_hour, rtc_tm.tm_min,rtc_tm.tm_sec);
	if (nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, test_csv_file_path, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
	snprintf(test_csv_file_path, sizeof(test_csv_file_path),
		 "%s-%04d%02d%02d_%02d%02d%02d.csv", SHORT_TEST_CSV_FILE, 
		   rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		   rtc_tm.tm_hour, rtc_tm.tm_min,rtc_tm.tm_sec);
	if (nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, test_csv_file_path, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
...
//+bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
int32_t nvt_selftest(struct seq_file *s, void *v);
int32_t nvt_selftest(struct seq_file *s, void *v)
{
	struct device_node *np = ts->client->dev.of_node;
	unsigned char mpcriteria[32] = {0}; //novatek-mp-criteria-default
	...
	TestResult_Final = 0;	//OAK497,shenwenbin.wt,MOD,20220112,selftest fail alway fail
	TestResult_Short = 0;
	TestResult_Open = 0;
	TestResult_FW_Rawdata = 0;
	TestResult_FW_CC = 0;
	...
	TestResult_Final |= TestResult_FW_Rawdata;
	NVT_LOG("TestResult_Final |= TestResult_FW_Rawdata = %d\n",TestResult_Final |= TestResult_FW_Rawdata);
	...
	TestResult_Final |= TestResult_FW_CC;
	NVT_LOG("TestResult_Final |= TestResult_FW_CC = %d\n",TestResult_Final |= TestResult_FW_CC);
	...
	if (ts->pen_support) {
		...
		TestResult_Final |= TestResult_Pen_FW_Raw;
	} /* if (ts->pen_support) */
	...
	TestResult_Final |= TestResult_Noise;
	if (ts->pen_support)
		TestResult_Final |= TestResult_Pen_Noise;
	NVT_LOG("TestResult_Final |= TestResult_Noise = %d\n",TestResult_Final |= TestResult_Noise);
	...
	TestResult_Final |= TestResult_Short;

	NVT_LOG("TestResult_Final |= TestResult_Short = %d\n",TestResult_Final |= TestResult_Short);
	//---Open Test---
	if (nvt_read_fw_open(RawData_Open) != 0) {
		TestResult_Open = 1;	// 1:ERROR
	} else {
	...
	TestResult_Final |= TestResult_Open;
	NVT_LOG("TestResult_Final |= TestResult_Open = %d\n",TestResult_Final |= TestResult_Open);
	...
	NVT_LOG("TestResult_Final = %d\n",TestResult_Final);
	NVT_LOG("--\n");
	
	return TestResult_Final;

}
EXPORT_SYMBOL(nvt_selftest);
//-bug697279,shenwenbin.wt,MOD,20211022,bringup selftest function	
